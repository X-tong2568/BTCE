# B站动态监控系统 (BTCE 4.0) 项目说明

## 项目简介
BTCE 4.0 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ 双通道推送。当前监控目标：星瞳_Official（UID: 401315430）。
...
├── monitor.py               # 核心监控：动态发现 + 置顶评论检测 + 截图 + 推送
...
| 文件 | 作用 |
|------|------|
| `monitor.py` | Monitor 类：Playwright 浏览器驱动，主循环每轮执行 ①置顶评论变更检测 ②API 动态列表发现新动态 ③卡片截图 → 邮件/QQ 推送 |
...
| `qq_message_generator.py` | QQMessageGenerator 类：生成 QQ 群消息，表情→alt文字，图片→CQ码，支持降级模式 |
...
main.py 启动
  ...
  │   ├─ 动态监控 (Monitor.run)
  │   │   └─ 每8秒循环:
  │   │       ├─ Playwright 浏览器打开置顶动态页 → 抓取置顶评论 → 变更检测 → 邮件+QQ
...
```python
# 监控目标
UP_UID = "401315430"                              # B站UID
PINNED_DYNAMIC_ID = "1199636880383016962"         # 手动置顶动态ID
