# B站动态监控系统 (BTCE 4.1) 项目说明

## 项目简介
BTCE 4.1 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ 双通道推送。当前监控目标：星瞳_Official（UID: 401315430）。
...
| 文件 | 作用 |
|------|------|
| `monitor.py` | Monitor 类：Playwright 浏览器驱动，主循环每轮执行 ①置顶评论变更检测+截图 ②API 动态列表发现新动态 ③卡片截图 → 邮件/QQ 推送 |
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.1
- **更新内容**：置顶评论高DPI截图 + 截图替换文字图片格式 + 失败兜底旧格式
- **Python 要求**：3.8+
- **最后更新**：2026年5月29日
