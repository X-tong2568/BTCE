# B站动态监控系统 (BTCE 4.6) 项目说明

## 项目简介
BTCE 4.6 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID。v4.6 新增直播通知附加房间状态标签（room_init API）。监控目标通过 `dynamic.py` 配置。
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.6
- **更新内容**：直播通知附加房间状态标签（room_init API + encrypted/锁房/隐藏/付费/拜年纪），纯展示不触发独立通知
- **Python 要求**：3.8+
- **最后更新**：2026年6月9日
