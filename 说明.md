# B站动态监控系统 (BTCE 4.3) 项目说明

## 项目简介
BTCE 4.3 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。监控目标通过 `dynamic.py` 配置。
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.3
- **更新内容**：三通道推送模式可配置（text/screenshot）+ 截图延迟到B站发布不阻塞通知
- **Python 要求**：3.8+
- **最后更新**：2026年5月30日
