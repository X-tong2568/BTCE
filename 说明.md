# B站动态监控系统 (BTCE 4.4) 项目说明

## 项目简介
BTCE 4.4 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID。监控目标通过 `dynamic.py` 配置。
...
| 文件 | 作用 |
|------|------|
| `monitor.py` | Monitor 类：Playwright 浏览器驱动，主循环每轮执行 ①置顶评论变更检测 ②API 动态列表发现新动态 ③按 QQ_MODE/EMAIL_MODE/BILI_MODE 分流推送 ④B站自动发布（截图延迟到通知后） |
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.5
- **更新内容**：修复 B站旧版 API 404 下线问题（更换 polymer 新版接口）；API 独立健康统计 + P1/P2 告警 + 日报双通道
- **Python 要求**：3.8+
- **最后更新**：2026年6月2日
