# B站动态监控系统 (BTCE 4.5) 项目说明

## 项目简介
BTCE 4.5 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID。v4.5 新增 API 独立健康统计 + 双通道 P1/P2 告警 + 日报分区展示。监控目标通过 `dynamic.py` 配置。
